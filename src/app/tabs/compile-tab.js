...
class CompileTab {
  ...
  constructor (editor, config, renderer, swarmfileProvider, fileManager, fileProviders, pluginManager) {
    ...
    this.compileTabLogic = new CompileTabLogic(self.queryParams, self.fileManager, self.editor, self.config, self.fileProviders)
    this.compiler = this.compileTabLogic.compiler
    this.compileTabLogic.init()

    this.compilerContainer = new CompilerContainer(self.compileTabLogic, self.editor, self.config, self.queryParams)
  ...
  listenToEvents () {
    ...
    self.compiler.event.register('compilationFinished', (success, data, source) => {
      ...
      if (data['error']) {
        error = true
        self.renderer.error(data['error'].formattedMessage, self._view.errorContainer, {type: data['error'].severity || 'error'})
      ...
      if (data.errors && data.errors.length) {
        ...
        data.errors.forEach((err) => {
          if (self.config.get('hideWarnings')) {
            if (err.severity !== 'warning') {
              self.renderer.error(err.formattedMessage, self._view.errorContainer, {type: err.severity})
            }
          } else {
            self.renderer.error(err.formattedMessage, self._view.errorContainer, {type: err.severity})
          }
      ...
      if (!error && data.contracts) {
        self.compiler.visitContracts((contract) => {
          self.renderer.error(contract.name, self._view.errorContainer, {type: 'success'})
        })
      }
    })
  ...
  publish () {
    ...
    if (selectContractNames.children.length > 0 && selectContractNames.selectedIndex >= 0) {
      ...
      if (contract.metadata === undefined || contract.metadata.length === 0) {
        ...
        publishOnSwarm(contract, self.fileManager, function (err, uploaded) {
          if (err) {
            try {
          ...
          self.swarmfileProvider.addReadOnly(item.hash, item.content)
        })
      }
    }
  }
    ...
    const contractEl = yo`
      <div class="${css.container}">
        <div class="${css.contractContainer}">
          ${this._view.contractNames}
          <div title="Publish on Swarm" class="${css.publish}" onclick=${() => { this.publish() }}>
            <i class="${css.copyIcon} fa fa-upload" aria-hidden="true"></i><span>Swarm</span>
          </div>
        </div>
        <div class="${css.contractHelperButtons}">
          <div title="Display Contract Details" class="${css.details}" onclick=${() => { this.details() }}>Details</div>
          <div title="Copy ABI to clipboard" class="${css.copyButton}" onclick=${() => { this.copyContractProperty('abi') }}>
            <i class="${css.copyIcon} fa fa-clipboard" aria-hidden="true"></i> ABI
          </div>
          <div title="Copy Bytecode to clipboard" class="${css.copyButton} ${css.bytecodeButton}" onclick=${() => { this.copyContractProperty('bytecode') }}>
  ...
  render () {
    ...
    this._view.errorContainer = yo`<div class='error'></div>`
    this._view.errorContainerHead = yo`<div class='error'></div>`
    this._view.contractNames = yo`<select class="${css.contractNames}" disabled></select>`

    this._view.el = yo`
      <div class="${css.compileTabView}" id="compileTabView">
        ${this.compilerContainer.render()}
        ${this.generateContractEl()}
